...
public class IPv4Network : IPNetwork, IEquatable<IPv4Network>
{
    ...
    public IPv4Network(byte[] bytes, int cidr)
        : base(bytes, cidr)
    {
        if (bytes.Length != 4)
            throw new ArgumentException("An IPv4 address always consists of 32 bits (4 bytes)", nameof(bytes));
        if (cidr < 0 || cidr > 32)
            throw new ArgumentOutOfRangeException(nameof(cidr), "CIDR must be between 0 and 32 (both inclusive)");
    }

    public IPv4Network(uint value, int cidr)
        : this([
            (byte)(value >> 24),
            (byte)((value >> 16) & 0xFF),
            (byte)((value >> 8) & 0xFF),
    ...
    public IPv4Network(string addressString, int cidr)
    ...
    public override IPNetwork[] SummarizeAddressRangeWith(IPNetwork other)
    {
        if (other is IPv4Network ip)
        {
            ...
            {
                int nbits = Math.Min(
                    Util.CountRighthandZeroBits32(first),
                    Util.BitLength32(last - first + 1) - 1
                );
    ...
    public static byte[] ParseAddressString(string addressString)
    {
        ...
        return [.. parts.Select(x => byte.Parse(x))];
    }

    public static bool operator ==(IPv4Network? a, IPv4Network? b)
    {
        if (a is null)
            return b is null;
